...
type unite_lexicale = 
    UL_IDENT of string
  | UL_OUVR | UL_FERM
  | UL_SUP | UL_INF | UL_EGAL | UL_DIFF | UL_INFEGAL | UL_SUPEGAL
  | UL_ET | UL_OU
  | UL_SI | UL_ALORS | UL_SINON | UL_FSI
  | UL_EOF
  | UL_ERR of string;;
...
let (mon_fichier : in_channel) = open_in "test3";;
  ...
  fun fichier automate e s -> let (charlu : char) = (try input_char(fichier) with End_of_file -> '\000') in
					   ...
					   | _ -> 
					          let (new_e : etat) = (trans e (type_du_carac(charlu))) in
						       ...
						       | Q_EOF          -> parcours_automate fichier automate new_e (String.make 1 charlu)
