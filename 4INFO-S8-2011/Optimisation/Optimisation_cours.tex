...
$\rho^{(n)})$ est optimal dans la direction $(w^{(n)})$.
...
\[ (\phi' \textit{ optimal }) \Ra \phi'(\rho_{n-1})=0 \]
Donc \[ <Ax^{(n)}-b,w^{(n-1)}>=0 \]
...
\begin{remarque}
Cette proposition suggère le principe suivant : si on connait $x^{(0)}, x^{(1)}, \dots, x^{(n)}$ et $w^{(0)}, w^{(1)}, \dots, w^{(n)}$, on cherche ... %TODO
\end{remarque}
...
\begin{enumerate}
\item si $r^{(0)}=0$, alors $Ax^{(0)}-b$ et ... %TODO
\item 
\end{enumerate}
...
Soit $x$ et $y$ dans $\R^N$, d'après Taylor-Lagrange, on a : \[ f(y)=f(x)+<\nabla f(x),y-x>+\frac{1}{2}<\nabla^2 f(x+\theta(y-x))(y-x),y-x> \quad (0<\theta<1) \]
\[ f(y)\geq f(x)+<\nabla f(x),y-x>+\frac{\alpha}{2}|y-x|^2 \quad (*)\quad \Ra f \textit{ est strictement convexe}\]
